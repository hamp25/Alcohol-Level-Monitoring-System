...
<body>
    <?php
        ...
        $sql = "SELECT * FROM arduino_tbl ORDER BY id DESC LIMIT 1";
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) { //fetch_assoc fetches a row from the result of our query iteratively for every row in our result.
                ...
                echo '<h1>Arduino BAC Logging</h1>';
                echo '<p>';
                echo '   <span class="dht-labels">BAC = </span>';
                echo '   <span id="temperature">'.$row["BAC"].' %</span>';
                echo ' </p>';
                echo '</div>';
            }
        } else {
